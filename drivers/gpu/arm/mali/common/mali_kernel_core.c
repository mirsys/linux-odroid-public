...
static _mali_osk_errcode_t mali_parse_product_info(void)
{
	...
	/* Find the first PP core resource */
...
static void mali_use_default_pm_domain_config(void)
{
	...
	/* GP core */
	if (_MALI_OSK_ERR_OK == _mali_osk_resource_find_by_id(MALI_RESOURCE_INDEX_GP, NULL)) {
...
static _mali_osk_errcode_t mali_parse_config_dma(void)
{
	...
	if (_MALI_OSK_ERR_OK == _mali_osk_resource_find_by_id(MALI_RESOURCE_INDEX_DMA, &resource_dma)) {
